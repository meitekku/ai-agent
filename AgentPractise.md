...
上面这些模式解决的是控制流怎么搭，下面再看另一个更工程的问题，系统为什么能跑稳。2. 为什么 Harness 比模型更关键
...
Harness 要做的就是把任务推进右上角，让对错有机器可以执行的判断标准，而不是靠人盯。3. 上下文工程为什么决定稳定性
...
解决方式是在框架层分两种消息类型：给应用层用的 AgentMessage 可以携带任意自定义字段，真正发给 LLM 的 Message 只保留 user、assistant、tool_result 三种标准类型，调用前过滤一遍，会话历史保留完整框架状态，LLM 只收它需要的部分。5. 记忆系统如何设计
...
4. Current Session：当前对话滑动窗口，不持久化
   OpenClaw 混合检索
5. memory/YYYY-MM-DD.md，追加写日志，保留原始细节
6. MEMORY.md，精选事实，Agent 主动维护
7. memory_search，70% 向量相似度 + 30% 关键词权重的混合检索
...
agent.on("event") -> send_to_eval_framework 10. 用 OpenClaw 看 Agent 如何落地
...
4. 上下文+记忆：Skills 延迟加载 + MEMORY.md，50% token 阈值自动整合
   消息总线如何把渠道和 Agent 隔开
...
class MessageBus {
async consumeInbound() { /_ 从队列取下一条消息 _/ }
async publishOutbound(msg) { /_ 路由到对应渠道发出 _/ }
}
...
第一个失败就建评测，把第一个真实失败案例转成测试用例，不要等积累够了再开始 11. Agent 落地里的常见反模式
...
5. 过早引入多 Agent：协调开销超过并行收益，先验证单 Agent 上限再扩展
6. 约束靠期望不靠机制：规则在文档里 Agent 选择性遵守，改用工具验证 / Linter / Hook
7. 收尾一下
