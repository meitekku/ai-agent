...
async function generateRationale(data: R, availableTemplateServices: string[]): Promise<AnalysisRationale> {
...
app.post("/deals/analyze", async (c) => {
  try {
    const { data, availableTemplateServices } = await c.req.json();
    if (!data?.opportunity) {
    ...
    if (process.env.GEMINI_API_KEY) {
      rationale = await generateRationale(data, templateServices);
    } else {
      rationale = {
