    ...
    tableRows.push([
      {
        text: `ポイント ${i + 1}`,
        options: {
          fill: i % 2 === 0 ? COLORS.tableRowEven : COLORS.tableRowOdd,
          color: COLORS.text,
          bold: true,
        },
      },
...
export async function POST(req: Request) {
  try {
    ...
    if (pngs.length > 0) {
      if (!pngs.every(isImageDataUrl)) {
        return new Response("Invalid image data URLs", { status: 400 });
      }
    ...
    for (let idx = 0; idx < slides.length; idx++) {
      ...
      const diagramPng =
        diagramPngs[idx] && isImageDataUrl(diagramPngs[idx] as string)
