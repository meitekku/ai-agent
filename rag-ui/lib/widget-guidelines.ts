...
### ネットワーク
Widget 内から **fetch / XMLHttpRequest / WebSocket が使用可能**。外部 API（Google Maps、天気、為替等）を直接呼び出せる。ただし Cookie や認証情報は送信されない（sandbox 制約）。
...
- **地図**: Google Maps（Embed API / Maps JavaScript API）、Leaflet + OpenStreetMap、静的 SVG 地図
- **3D・アニメーション**: Three.js（CDN: cdn.jsdelivr.net/npm/three）でシーン・モデル・パーティクル
...
- **ゲーム・デモ**: Canvas/WebGL ゲーム、パズル、記憶ゲーム、物理シミュレーション、Three.js 3D ゲーム
- **カレンダー**: 月間カレンダー、スケジュール表、イベントタイムライン
