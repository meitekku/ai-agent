...
export function HtmlSlideViewer({
  ...
  // Plan diagnostics
  const [planDiag, setPlanDiag] = useState<{
    source?: string;
    model?: string;
    error?: string;
    prompt_len?: number;
  ...
  const fetchPlan = useCallback(async () => {
    ...
    try {
      const hasStyle = !!(
        styleOptions.industry ||
        styleOptions.profession ||
        styleOptions.ageGroup ||
    ...
    if (useTemplates) {
      try {
        ...
        if (!templates || templates.length === 0) {
          setTemplateWarning(
            "テンプレートがアップロードされていません。「管理」からアップロードしてください。",
          );
          return;
        }
      } catch {
                ...
                {planDiag && (
                  ...
                  >
                    <span className="font-bold">
                      {planDiag.source === "llm" ? "LLM生成" : "フォールバック"}
                    </span>
              ...
              <div className="space-y-1.5">
                ...
                <div className="h-2 bg-secondary rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${generatingTotal > 0 ? (generatingCompleted / generatingTotal) * 100 : 0}%`,
                    }}
                  />
                </div>
              </div>
